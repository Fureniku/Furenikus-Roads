...
public class ChevronPaintBlock extends PaintBlockBase {
	...
	public ChevronPaintBlock(String name, EnumJunctionConnections type, boolean aType) {
		super(name);
		this.setDefaultState(this.blockState.getBaseState().withProperty(META_ID, EnumType.NORTH).withProperty(TYPE, type).withProperty(EDGE, false));
		if (!aType) { this.setCreativeTab(null); }
	}
	...
	protected BlockStateContainer createBlockState() {
		return new BlockStateContainer(this, new IProperty[] {META_ID, TYPE, EDGE});
	}
	
	@Override
	public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
		return state.withProperty(EDGE, connectToLine(state, worldIn, pos));
	}
