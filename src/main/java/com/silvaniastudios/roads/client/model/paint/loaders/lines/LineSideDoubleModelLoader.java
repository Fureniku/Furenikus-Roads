...
public class LineSideDoubleModelLoader implements ICustomModelLoader {
	
	public static final PaintLineSideDoubleModel MODEL = new PaintLineSideDoubleModel();
	
	@Override
	public boolean accepts(ResourceLocation modelLocation) {
		if (modelLocation.getResourceDomain().equals(FurenikusRoads.MODID)) {
			for (int i = 0; i < FRBlocks.col.length; i++) {
				if (modelLocation.getResourcePath().equals("line_" + FRBlocks.col[i].getName() + "_side_double")) {
					return true;
				}
			}
		}
...
class PaintLineSideDoubleModel extends PaintModelBase {
...
class PaintLineSideDoubleBakedModel extends PaintBakedModelBase {
