...
public class LineSideSingleModelLoader implements ICustomModelLoader {
	
	public static final PaintLineSideSingleModel MODEL = new PaintLineSideSingleModel();
	
	@Override
	public boolean accepts(ResourceLocation modelLocation) {
		if (modelLocation.getResourceDomain().equals(FurenikusRoads.MODID)) {
			for (int i = 0; i < FRBlocks.col.length; i++) {
				if (modelLocation.getResourcePath().equals("line_" + FRBlocks.col[i].getName() + "_side_single")) {
					return true;
				}
			}
		}
...
class PaintLineSideSingleModel extends PaintModelBase {
...
class PaintLineSideSingleBakedModel extends PaintBakedModelBase {
