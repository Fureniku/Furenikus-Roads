...
public class PaintLineThinCrossingModel extends PaintModelBase {

    @Override
    public IBakedModel bake(IModelState state, VertexFormat format, Function<ResourceLocation, TextureAtlasSprite> bakedTextureGetter) {
        return new PaintLineThinCrossingBakedModel(state, format, bakedTextureGetter);
    }
}

class PaintLineThinCrossingBakedModel extends PaintLineBakedModelBase {

    public PaintLineThinCrossingBakedModel(IModelState state, VertexFormat format, Function<ResourceLocation, TextureAtlasSprite> bakedTextureGetter) {
        ...
        if (state != null) {
            ...
            if (sprite != null) {
                switch (facing) {
                    case NORTH:
                    case SOUTH:
                        rawQuads.addAll(shapeLine(format, sprite, v*3f, v*0, v*13f, v*16, 0f, 0f, 0f, 0f));
                        break;
                    case EAST:
                    case WEST:
                        rawQuads.addAll(shapeLine(format, sprite, v*0f, v*3f, v*16f, v*13f, 0f, 0f, 0f, 0f));
                        break;
                }
